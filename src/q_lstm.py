...
#auxCols = dataIn.columns
dataIn = pd.DataFrame(scaler.fit_transform(dataIn), columns=[ride])
...
X = waitTimesLagged.iloc[:,horizon:]
...
model = Sequential()
model.add(LSTM(4, input_shape=(1,samplesForPrediction)))
model.add(Dense(1))
